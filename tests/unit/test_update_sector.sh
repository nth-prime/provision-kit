...
assert_contains "$UPDATE_SECTOR" 'SRC_DIR=' "Update sector must resolve extracted source directory"
assert_contains "$UPDATE_SECTOR" 'bash "\./install.sh"' "Update sector must run installer from extracted source directory"
