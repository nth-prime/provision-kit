...
assert_contains "$USER_SSH" 'Linux/macOS:' "User SSH sector must provide Linux/macOS public key instructions"
assert_contains "$USER_SSH" 'ssh-keygen -C' "User SSH sector must provide key generation instruction"
assert_contains "$USER_SSH" 'Get-Content \$env:USERPROFILE' "User SSH sector must provide Windows public key path instruction"

assert_contains "$ROTATE_SSH" 'Linux/macOS:' "Rotate sector must provide Linux/macOS public key instructions"
assert_contains "$ROTATE_SSH" 'ssh-keygen -C' "Rotate sector must provide key generation instruction"
